...
struct Atom {
    ...
    if(mathOps.canFind(op)) {
        if(isNumeric && rhs.isNumeric) {
            ...
            static foreach(Type; FiniteIntegralTypes) {
                if(isType!Type || rhs.isType!Type || is(Type == real) && (
                    op == "/" && this % rhs != Atom(0) || op == "^^" && rhs < Atom(0)
                )) {
                    Type a = this.as!Type;
                    Type b = rhs.as!Type;
                    static if(op == "%") {
    ...
    Atom binaryFallback(string op : "+")(Atom rhs) {
        return match!(
            ...
            (Atom[] a, b) => Atom(a ~ [atomFor(b)]),
            (a, Atom[] b) => Atom([atomFor(a)] ~ b),
    ...
    Atom binaryFallback(string op : "%")(Atom rhs) {
        return match!(
            ...
            (a, Atom[] b) => Atom(uninterleave(b, a).map!Atom.array),
            (a, string b) => Atom(uninterleave(b.atomChars, a).map!joinToString.map!Atom.array),
            (_1, _2) => Nil.nilAtom,
        )(this, rhs);
    }
